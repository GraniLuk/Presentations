...
```mermaid
flowchart LR
    A[🎹 Press Shortcut] --> B[🎙️ Record Audio]
    B --> C[🔇 VAD Filters Silence]
    C --> D[🧠 Whisper / Parakeet]
    D --> E[📋 Text Pasted]
```
...
flowchart TB
    ...
    subgraph Backend["⚙️ Backend (Rust)"]
        Audio[🎙️ Audio I/O<br/>cpal]
        VAD[🔇 VAD<br/>vad-rs / Silero]
        Whisper[🧠 whisper-rs<br/>Whisper Models]
        Parakeet[🧠 transcription-rs<br/>Parakeet Models]
        KB[⌨️ Global Shortcuts<br/>rdev]
        Resample[🔊 Resampling<br/>rubato]
    end

    subgraph Tauri["🦀 Tauri v2"]
        Bridge[IPC Bridge]
    end
...
flowchart TB
    ...
    Cloud -.- |"❌ Privacy concerns<br/>💰 API costs<br/>🌐 Internet required"| Note1[ ]
    Local -.- |"✅ Privacy<br/>🆓 Free<br/>📴 Offline"| Note2[ ]

    style Handy fill:#f77f00,color:#1a1a2e,stroke:#f77f00
    style Note1 fill:transparent,stroke:transparent,color:#eaeaea
    style Note2 fill:transparent,stroke:transparent,color:#eaeaea
```
